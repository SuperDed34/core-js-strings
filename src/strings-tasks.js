...
function isPalindrome(str) {
  return str
    ...
    .reverse()
    .join('')
    .includes(str.toLocaleLowerCase().replaceAll(/[^\w]|_/g, ''));
}
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((acc, item) => {
    const res = acc.length < item.length ? item : acc;
    return res;
  });
}
...
function encodeToRot13(str) {
  return str
    ...
    .map((item) => {
      let res = '';
      if (
        (item.charCodeAt(0) >= 65 && item.charCodeAt(0) <= 77) ||
        (item.charCodeAt(0) >= 97 && item.charCodeAt(0) <= 109)
      ) {
        res = String.fromCharCode(item.charCodeAt(0) + 13);
        return res;
      }
      if (
        (item.charCodeAt(0) >= 78 && item.charCodeAt(0) <= 90) ||
